...
func (h *PublicHandler) ListPublicTableData(c *gin.Context) {
	...
	if err != nil {
		respondWithError(c, http.StatusInternalServerError, "failed to query table", err)
		return
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.MapScan(row); err != nil {
			respondWithError(c, http.StatusInternalServerError, "failed to scan rows", err)
			return
		}
	...
	if err := rows.Err(); err != nil {
		respondWithError(c, http.StatusInternalServerError, "failed to read rows", err)
		return
	}
